#!/usr/bin/env python3
"""Grokking grid: 8 algebras × 3 weight decay × 8 seeds, mandatory --zheng-protocol.

    uv run python scripts/grok_grid.py --parallel 6 --device cuda
    uv run python scripts/grok_grid.py --summarize
    uv run python scripts/grok_grid.py --link-existing
    uv run python scripts/grok_grid.py --dry-run
"""
...
OUT_JSON = ROOT / "new_paper_rings" / "grok_grid_results.json"
OUT_TEX = ROOT / "new_paper_rings" / "generated" / "grok_grid_rate.tex"
OUT_TEX_ROWS = ROOT / "new_paper_rings" / "generated" / "grok_grid_rows.tex"
LOG_DIR = ROOT / "logs" / "grok_grid"
...
def write_tex(payload: dict | None = None) -> Path:
    ...
    row_lines = [
        "% Auto-generated fragment for Table~\\ref{tab:grok-summary}b (scripts/grok_grid.py --summarize)",
    ...
    lines = [
        "% Auto-generated by scripts/grok_grid.py --summarize",
        r"% Legacy standalone table; main paper uses \input{generated/grok_grid_rows.tex} in tab:grok-summary.",
        r"\begin{table}[htbp]",
        r"\centering",
        (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Grokking grid: Zheng protocol + weight-decay sweep")
    ...
    parser.add_argument("--summarize", action="store_true", help="Aggregate checkpoints → grok_grid_results.json")
    ...
    jobs = all_jobs(rings=rings)
    print(f"Grokking grid: {len(jobs)} jobs, parallel={args.parallel}, device={args.device}")
    t0 = time.perf_counter()
    results = run_grid(
